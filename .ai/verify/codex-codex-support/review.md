...
| Severity | Location                                                    | Finding                                                                                                                                                                                                              | Suggested fix                                                                                                               |
| -------- | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| medium   | `.claude/hooks/hook-common.sh:1`                            | The live hooks are hand-maintained copies of the bootstrap templates — kept identical through both fix passes by manual `cmp`, but nothing enforces it, and hook-runtime.test.sh exercises only the template copies  | Add a `cmp` loop to validate-artifacts.sh (or symlink the live hooks to the templates, as `plugins/*/scripts/runtime` does) |
...
both hook copies (verified with `cmp`). The full validate suite is green
