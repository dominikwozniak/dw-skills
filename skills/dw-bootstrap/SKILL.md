...
name: dw-bootstrap
description: >-
  Scaffold a project for Claude Code and the `dw-spec → dw-plan → dw-build` loop — `.ai/` memory
  dirs, a tracked `.claude/settings.json` with guardrail hooks, a `CLAUDE.local.md`, and a cleaned
  `.gitignore`. Prefer `dw-init` for a repo only you read. Explicit-invoke only. Use when setting
  up a new repo for Claude Code or standardizing an existing one; trigger phrases: "set up this
  project", "bootstrap claude", "scaffold the agent files".
