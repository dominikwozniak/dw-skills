...
| `link-local-memory`        | always                          | stack-agnostic `SessionStart`; inside a `git worktree`, symlinks the main tree's gitignored `CLAUDE.local.md` in so `dw-git` and the lint/typecheck hooks still see it. No-op elsewhere      |
